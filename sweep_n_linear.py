...
with wandb.init(tags=['NLinear2']):
    ...
    normalize = config.normalize is True or config.normalize == "true" or config.normalize == "True"

    model = NLinearModel(input_chunk_length=config.input_chunk_length,  # Lookback window [15, 30, 40, 45]
                         output_chunk_length=4,
                         normalize=normalize,  # Hyperparameter [True, False]
    ...
    tft = NLinearModel.load_from_checkpoint(model_name=model.model_name, best=True)
