...
with wandb.init(tags=['TSMixer-Quantile']):
    config = wandb.config
    ...
    tsmixer = TSMixerModel(input_chunk_length=config.input_chunk_length,
    ...
    # Train model
    tsmixer.fit(series=targets[:train_end],
    ...
    # Evaluate model
    _ = evaluate_model(model=tsmixer,
                       targets=targets,
